...
namespace Econosys.Api.Controllers
{
    ...
    public class StockTakingItemsController : ControllerBase
    {
        ...
        private readonly ILogger<StockTakingItemsController> _logger;

        public StockTakingItemsController(ApplicationDbContext dbContext, ILogger<StockTakingItemsController> logger)
        {
        ...
        [HttpGet("{id:int}")]
        public async Task<ActionResult<StockTakingItemDto>> GetById(int id)
        {
            var stockTakingItem = await _dbContext.StockTakingItems
                .AsNoTracking()
                .FirstOrDefaultAsync(x => x.Id == id);

            if (stockTakingItem is null)
            {
                return NotFound();
            }

            return Ok(MapToDto(stockTakingItem));
        }

        [HttpPost("search")]
        public async Task<ActionResult<PagedResultDto<StockTakingItemDto>>> Search([FromBody] SearchStockTakingItemsRequest? request)
        {
            IQueryable<StockTakingItem> query = _dbContext.StockTakingItems.AsNoTracking();
            ...
            if (request?.Filter?.Conditions?.Count > 0)
            {
                foreach (var condition in request.Filter.Conditions)
                {
                    ...
                    {
                        _logger.LogWarning(ex, "Invalid stock taking item search condition for field {Field}", condition.Field);
                        return BadRequest(new { message = ex.Message, field = condition.Field });
                    }
                }
            }

            IOrderedQueryable<StockTakingItem> orderedQuery;
            try
            {
            ...
            {
                _logger.LogWarning(ex, "Invalid stock taking item order by request");
            ...
            return Ok(new PagedResultDto<StockTakingItemDto>
            {
                Items = items,
        ...
        private static Dictionary<string, PropertyInfo> BuildFieldMap()
        {
            ...
            foreach (var property in typeof(StockTakingItem).GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
        ...
        private static IOrderedQueryable<StockTakingItem> ApplyOrdering(
            IQueryable<StockTakingItem> query,
            List<SortRequest>? orderBy)
        {
            ...
            IOrderedQueryable<StockTakingItem>? ordered = null;
        ...
        private static IOrderedQueryable<StockTakingItem> ApplySort(
            IOrderedQueryable<StockTakingItem>? ordered,
            IQueryable<StockTakingItem> source,
            PropertyInfo property,
            bool isDescending)
        {
            var parameter = Expression.Parameter(typeof(StockTakingItem), "x");
            ...
            var method = typeof(Queryable)
                ...
                .MakeGenericMethod(typeof(StockTakingItem), property.PropertyType);

            var result = method.Invoke(null, new object[] { ordered ?? source, lambda });
            return (IOrderedQueryable<StockTakingItem>)result!;
        }

        private static IQueryable<StockTakingItem> ApplyCondition(IQueryable<StockTakingItem> query, FilterConditionDto condition)
        {
            ...
            var lambda = Expression.Lambda<Func<StockTakingItem, bool>>(body, parameter);
        ...
        private static StockTakingItemDto MapToDto(StockTakingItem source)
        {
            return new StockTakingItemDto
            {
                Id = source.Id,
                StockTakingId = source.StockTakingId,
                SupplierOrderId = source.SupplierOrderId,
                NrOfItems = source.NrOfItems,
                NrOfPallets = source.NrOfPallets,
                DiffNrOfItems = source.DiffNrOfItems,
                DiffNrOfPallets = source.DiffNrOfPallets,
