...
const mapDtoToForm = (dto) => ({
    ...
    typeOf: parseNullableInt(dto?.typeOf),
    paymentDays: parseNullableInt(dto?.paymentDays),
});
...
const mapFormToPayload = (form) => ({
    ...
    typeOf: parseNullableInt(form.typeOf),
    paymentDays: parseNullableInt(form.paymentDays),
});
...
const buildDeleteConflictMessage = (error) => {
    ...
    return 'Kunde inte radera betalningsvillkor.';
};
...
const PaymentTermsSettings = () => {
    ...
    const loadList = async () => {
        ...
        try {
            const response = await apiClient.get('/termsofpayment/search');
            setItems(response?.data ?? []);
            setMessage({ type: 'info', text: "Betalningsvillkor hamtas fran API:t." });
        } catch (error) {
            console.error('Failed to load payment terms list:', error);
            setMessage({ type: 'error', text: 'Kunde inte hamta betalningsvillkor.' });
        } finally {
    ...
    const selectItem = async (id) => {
        ...
        try {
            const response = await apiClient.get(`/termsofpayment/${id}`);
            const mapped = mapDtoToForm(response.data);
            setForm(mapped);
            setOriginalForm(mapped);
            setTranslationTemplate(mapped.translations);
        } catch (error) {
            console.error('Failed to load payment term details:', error);
            setMessage({ type: 'error', text: 'Kunde inte hamta betalningsvillkor.' });
        } finally {
    ...
    const handleCreateNew = () => {
        if (hasUnsavedChanges) {
            setMessage({ type: 'error', text: 'Spara eller radera innan du skapar ett nytt betalningsvillkor.' });
            return;
        }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const response = form.id
                ? await apiClient.put(`/termsofpayment/${form.id}`, payload)
                : await apiClient.post('/termsofpayment', payload);
            ...
            setIsCreateMode(false);
            await loadList();
            setMessage({ type: 'success', text: 'Betalningsvillkor sparades.' });
        } catch (error) {
            console.error('Failed to save payment term:', error);
            setMessage({ type: 'error', text: 'Kunde inte spara betalningsvillkor.' });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            await apiClient.delete(`/termsofpayment/${form.id}`);
            await loadList();
            setForm(defaultForm);
            setOriginalForm(defaultForm);
            setTranslationTemplate([]);
            setIsCreateMode(false);
            setMessage({ type: 'success', text: 'Betalningsvillkor raderades.' });
        } catch (error) {
    ...
    return (
        <div className="relative flex flex-col h-full">
            <ConfirmationModal
                ...
                title="RADERA BETALNINGSVILLKOR"
                message={`Ar du saker pa att du vill radera betalningsvillkor ${form.name || form.id}? Atgarden kan inte angras.`}
                confirmText={isDeleting ? 'Raderar...' : 'Radera'}
                cancelText="Avbryt"
                isDestructive={true}
            />

            <h2 className="ml-5 text-sm pt-2 pb-2 text-gray-700">Betalningsvillkor</h2>
                ...
                <div className="w-[450px] shrink-0 mb-5 px-4 py-2 border-r border-gray-300">
                    ...
                    <div className="mt-5 border-t border-gray-200 pt-2 space-y-0.5 max-h-[calc(100vh-230px)] overflow-y-auto">
                        {isLoadingList ? (
                            ...
                            <p className="text-xs text-gray-500 py-2">Inga betalningsvillkor</p>
                        ) : (
                ...
                <div className="flex-1 min-w-0 px-10 py-2 overflow-x-auto">
                    ...
                    <div className={`w-80 min-w-0 mt-3 ${isEditDisabled ? 'opacity-70' : ''}`}>
                        ...
                        <LabeledInput
                            ...
                            margintop="0"
                            value={form.typeOf ?? ''}
                            onChange={(value) => updateField('typeOf', value ?? '')}
                            disabled={isEditDisabled}
                            maxLength={10}
                        />
                        ...
                        <LabeledInput
                            ...
                            onChange={(value) => updateField('paymentDays', value ?? '')}
                            disabled={isEditDisabled}
                            maxLength={10}
                        />
                        ...
                        <LabeledSwitch
                            id="payment-term-active"
                            name="payment-term-active"
                            label="Aktiv"
                            labelWidth="w-20"
                            marginTop={4}
                            value={form.active}
