...
const defaultForm = {
    ...
    isDefault: false,
    active: true,
    nrOfCalcDecimals: null,
    nrOfCalcDecimalsQty: null,
};
...
const mapDtoToForm = (dto) => ({
    ...
    multiplicator: dto?.multiplicator ?? null,
    isDefault: Boolean(dto?.isDefault),
    active: Boolean(dto?.active),
    nrOfCalcDecimals: dto?.nrOfCalcDecimals ?? null,
    nrOfCalcDecimalsQty: dto?.nrOfCalcDecimalsQty ?? null,
});
...
const mapFormToPayload = (form) => ({
    ...
    multiplicator: parseNullableInt(form.multiplicator),
    isDefault: form.isDefault,
    active: form.active,
    nrOfCalcDecimals: parseNullableInt(form.nrOfCalcDecimals),
    nrOfCalcDecimalsQty: parseNullableInt(form.nrOfCalcDecimalsQty),
});
...
const buildDeleteConflictMessage = (error) => {
    ...
    return 'Kunde inte radera enhet.';
};
...
const UnitsSettings = () => {
    ...
    const loadList = async () => {
        ...
        try {
            const response = await apiClient.post('/units/search', {
                pagination: { pageNumber: 1, pageSize: 500 },
                orderBy: [{ field: 'id', direction: 'asc' }],
            });
            setItems(response?.data?.items ?? []);
        } catch (error) {
            console.error('Failed to load units list:', error);
            setMessage({ type: 'error', text: 'Kunde inte hamta enheter.' });
        } finally {
    ...
    const selectItem = async (id) => {
        ...
        try {
            ...
            setMessage({ type: 'error', text: 'Kunde inte hamta enhet.' });
        } finally {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const response = form.id
                ? await apiClient.put(`/units/${form.id}`, payload)
            ...
            setIsCreateMode(false);
            await loadList();
            setMessage({ type: 'success', text: 'Enhet sparades.' });
        } catch (error) {
            console.error('Failed to save unit:', error);
            setMessage({ type: 'error', text: 'Kunde inte spara enhet.' });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            await apiClient.delete(`/units/${form.id}`);
            await loadList();
            setForm(defaultForm);
            setOriginalForm(defaultForm);
            setIsCreateMode(false);
            setTranslationTemplate([]);
            setMessage({ type: 'success', text: 'Enhet raderades.' });
        } catch (error) {
    ...
    return (
        <div className="relative flex flex-col h-full">
            <ConfirmationModal
                ...
                title="RADERA ENHET"
                message={`Ar du säker på att du vill radera enhet ${form.name || form.id}? Åtgarden kan inte ångras.`}
            ...
            <h2 className="ml-5 text-sm pt-2 pb-2 text-gray-700">Enheter</h2>
                ...
                <div className="w-[450px] shrink-0 mb-5 px-4 py-2 border-r border-gray-300">
                    ...
                    <div className="mt-5 border-t border-gray-200 pt-2 space-y-0.5 max-h-[calc(100vh-230px)] overflow-y-auto">
                        {isLoadingList ? (
                            ...
                            <p className="text-xs text-gray-500 py-2">Inga enheter</p>
                        ) : (
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 items-start">
                        <div className={`min-w-0 mt-3 ${isEditDisabled ? 'opacity-70' : ''}`}>
                            ...
                            <LabeledInput
                                ...
                                onChange={(value) => updateField('multiplicator', value)}
                                disabled={isEditDisabled}
                            />
                            ...
                            <LabeledInput
                                ...
                                type="number"
                                integerOnly
                                value={form.nrOfCalcDecimals}
                                onChange={(value) => updateField('nrOfCalcDecimals', value)}
                                disabled={isEditDisabled}
                            />

                            <LabeledInput
                                label="Decimaler kvantitet"
                                labelWidth="w-24"
                                inputWidth="w-20"
                                margintop="0"
                                type="number"
                                integerOnly
                                value={form.nrOfCalcDecimalsQty}
                                onChange={(value) => updateField('nrOfCalcDecimalsQty', value)}
                                disabled={isEditDisabled}
                            />

                            <LabeledSwitch
                                id="unit-is-default"
                                name="unit-is-default"
                                label="Standard"
                                labelWidth="w-24"
                                marginTop={12}
