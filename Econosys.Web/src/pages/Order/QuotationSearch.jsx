...
const QuotationSearch = () => {
    ...
    const handleOpenQuotation = (event, quotationId) => {
        event.preventDefault();
        event.stopPropagation();
        navigate(`/order/quotations/${quotationId}`);
    };
                ...
                <table className="table-fixed w-full border-collapse text-xs" style={{ fontFamily: "'Neue Haas Unica', 'Helvetica Neue', Arial, sans-serif" }}>
                    ...
                    <tbody className={`${!loading && rows.length > 0 ? 'bg-white' : 'bg-transparent'}`}>
                        {showSkeleton ? (
                            ...
                            <tr>
                                <td colSpan={columns.length} className="px-4 py-8 text-center text-gray-400">Inga affarsforslag hittades.</td>
                            </tr>
                        ) : (
                            rows.map((row) => (
                                <tr
                                ...
                                >
                                    <td className="truncate px-2 py-1 text-gray-800">
                                        <button
                                            type="button"
                                            onClick={(event) => handleOpenQuotation(event, row.id)}
